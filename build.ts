...
export type RustModuleCompiler = (
	sourcePath: string,
...
type ImportKind = 'static' | 'dynamic' | 'require';
 ...
 */
export async function prepareRustImports(
	options: PrepareRustImportsOptions
): Promise<PrepareRustImportsResult> {
	...
	const artifactsBySource = new Map<string, RustArtifact>();
	const queue = [...entrypoints];
	const queued = new Set(queue);

	for (let cursor = 0; cursor < queue.length; cursor += 1) {
		const filePath = queue[cursor];
		if (records.has(filePath)) continue;
		...
		for (let index = 0; index < scanned.length; index += 1) {
			...
			if (!isLocalSpecifier(item.path)) continue;
			...
			if (rustImport != null) {
				...
				if (rewrite === 'in-place') {
					...
					if (!token) {
						throw new Error(
							`Could not safely locate the ${JSON.stringify(item.path)} import token in ${filePath}`
						);
					}
...
function scanImports(filePath: string, source: string): ScannedImport[] {
	const transpiler = new Bun.Transpiler({ loader: loaderFor(filePath) });
	return transpiler.scanImports(source);
}

function loaderFor(filePath: string): 'js' | 'jsx' | 'ts' | 'tsx' {
...
async function resolveRustImport(importer: string, specifier: string, rootDir: string): Promise<string> {
...
async function resolveSourceImport(
	...
	if (requestedExtension === '') {
		for (const extension of EXTENSION_CANDIDATES) candidates.push(`${requested}${extension}`);
		for (const extension of EXTENSION_CANDIDATES) candidates.push(join(requested, `index${extension}`));
	...
	for (const candidate of candidates) {
		...
		if (!isSourceModule(candidate)) return null;
	...
	if (requestedExtension !== '' && !SOURCE_EXTENSIONS.has(requestedExtension)) return null;
...
function rustPathFromSpecifier(specifier: string): string | null {
...
function validateCompilerArtifact(artifact: RustArtifact, sourcePath: string): void {
	if (resolve(artifact.sourcePath) !== sourcePath) {
		throw new Error(
			`Rust compiler returned an artifact for ${artifact.sourcePath}, expected ${sourcePath}`
		);
	}
...
async function canonicalDirectory(path: string): Promise<string> {
	...
	if (!info.isDirectory()) throw new Error(`Rust import root is not a directory: ${resolved}`);
	return canonical;
}

async function canonicalFile(path: string, rootDir: string, label: string): Promise<string> {
	...
	const info = await stat(canonical);
	if (!info.isFile()) throw new Error(`${label} is not a file: ${canonical}`);
	return canonical;
}

function assertInsideRoot(rootDir: string, path: string, label: string): void {
...
async function transactionallyRewriteSources(
	rootDir: string,
	...
	try {
		...
		const message =
			`Failed to commit Rust import rewrites: ${errorMessage(error)}` +
...
async function restoreCommittedRewrite(rootDir: string, item: StagedRewrite): Promise<void> {
	...
	try {
		...
		try {
			...
			throw new Error(
				`Refusing to roll back ${item.path}; its path changed after this transaction committed it`,
				{ cause: error }
			);
		}
		if (canonical !== item.path || (await readFile(item.path, 'utf8')) !== item.replacement) {
			throw new Error(
				`Refusing to roll back ${item.path}; its contents or path changed after this transaction committed it`
			);
		}
...
async function validateRewriteTarget(
	rootDir: string,
	record: SourceRecord
): Promise<{ current: string; mode: number }> {
...
async function assertRewriteTargetUnchanged(
	rootDir: string,
	path: string,
	original: string
): Promise<void> {
...
function applyRewrites(path: string, source: string, changes: Rewrite[]): string {
...
function temporaryPathFor(path: string, phase: 'stage' | 'rollback'): string {
	return join(
		dirname(path),
		`.${basename(path)}.${process.pid}.${crypto.randomUUID()}.rust-imports-${phase}`
	);
}

async function cleanupStagedFiles(staged: StagedRewrite[]): Promise<void> {
	await Promise.allSettled(
		staged.filter((item) => !item.committed).map((item) => rm(item.temporaryPath, { force: true }))
	);
}
...
function quoteSpecifier(value: string, quote: "'" | '"' | '`'): string {
...
function matchScannedImports(
	scanned: ScannedImport[],
	tokens: SpecifierToken[]
): Array<SpecifierToken | undefined> {
	const matches: Array<SpecifierToken | undefined> = new Array(scanned.length);
	...
	for (let scanIndex = 0; scanIndex < scanned.length; scanIndex += 1) {
		...
		if (tokenIndex < 0) {
			tokenIndex = tokens.findIndex(
				(token, index) =>
					!used.has(index) && token.kind === kind && token.value === item.path
			);
		}
...
function scannedKind(kind: string): ImportKind | null {
	switch (kind) {
 ...
 */
function locateImportSpecifierTokens(source: string): SpecifierToken[] {
	...
	for (let index = 0; index < tokens.length; index += 1) {
		...
		if (token.value === 'import') {
			...
			if (from) specifiers.push(toSpecifierToken('static', from));
			continue;
		}
...
function findFromSpecifier(tokens: LexToken[], start: number): LexToken | null {
...
function toSpecifierToken(kind: ImportKind, token: LexToken): SpecifierToken {
...
function lexJavaScript(source: string): LexToken[] {
	...
	function scanTemplate(start: number): number {
		...
		while (index < source.length) {
			...
			if (char === '$' && source[index + 1] === '{') {
				if (!hasInterpolation) tokens.push({ kind: 'other', start, end: index + 2 });
...
function decodeStringContents(raw: string): string {
	let decoded = '';
...
function canStartRegularExpression(previous: LexToken | undefined): boolean {
...
function scanIdentifier(source: string, start: number): number {
	let index = start;
	while (index < source.length && isIdentifierContinue(source[index])) index += 1;
	return index;
}
