 ...
 */
export function rustImportsPlugin(options = {}) {
    const { rootDir = defaultRustRoot, ...compilerOptions } = options;
    const allowedRoot = resolve(rootDir);
    const compileOptions = { ...compilerOptions, allowedRoot };
    ...
    return {
        ...
        setup(build) {
            build.onResolve({ filter: /\.rs$/ }, (args) => {
                ...
                if (!isRustFileSpecifier(args.path))
                    return;
                ...
                try {
                    ...
                    throw withCause(`rust-imports: could not resolve ${JSON.stringify(args.path)}${importedFrom}: ${errorMessage(error)}`, error);
                }
            });
            build.onLoad({ filter: /\.rs$/, namespace }, async (args) => {
                ...
                try {
                    ...
                    throw withCause(`rust-imports: failed to prepare ${args.path}: ${errorMessage(error)}. ` +
                        'Ensure Cargo is available, or run the rust-imports build preparation step before starting Bun.', error);
                }
                let nativeExports;
                try {
                    ...
                    throw withCause(`rust-imports: compiled ${args.path}, but could not load ${artifact.nodePath} ` +
                        `for ${process.platform}-${process.arch}: ${errorMessage(error)}. ` +
                        'Rebuild the native artifact for this runtime and architecture.', error);
                }
            });
        }
    };
...
export function isRustFileSpecifier(specifier) {
    return (extname(specifier) === '.rs' &&
...
export function resolveRustImportPath(specifier, resolveDir, rootDir = defaultRustRoot) {
    if (!specifier)
        throw new Error('the Rust import specifier is empty');
    if (specifier.includes('\0'))
        throw new Error('the Rust import specifier contains a NUL byte');
    ...
    if (!isRustFileSpecifier(specifier)) {
        throw new Error(`Rust imports must be relative (./ or ../) or absolute; received ${JSON.stringify(specifier)}`);
    }
...
export function assertPathInsideRoot(sourcePath, rootDir) {
    const absoluteRoot = resolve(rootDir);
    const absoluteSource = resolve(sourcePath);
    const fromRoot = relative(absoluteRoot, absoluteSource);
    if (fromRoot === '..' || fromRoot.startsWith(`..${sep}`) || isAbsolute(fromRoot)) {
        throw new Error(`Rust source ${absoluteSource} is outside the allowed root ${absoluteRoot}; ` +
 ...
 */
export async function resolveRustSource(specifier, resolveDir, rootDir = defaultRustRoot) {
    ...
    if (!sourceStat.isFile())
        throw new Error(`Rust source is not a file: ${canonicalSource}`);
    return canonicalSource;
}
/** Only expose names Bun can safely materialize as named ESM exports. */
export function isValidJavaScriptIdentifier(name) {
 ...
 */
export function shapeNativeExports(nativeExports) {
    if (nativeExports === null ||
        (typeof nativeExports !== 'object' && typeof nativeExports !== 'function')) {
        throw new Error(`rust-imports: native addon returned ${nativeExports === null ? 'null' : typeof nativeExports}; ` +
            'expected an object or function export');
    }
    const result = { default: nativeExports };
    for (const name of Object.keys(nativeExports)) {
        if (name === 'default' || !isValidJavaScriptIdentifier(name))
            continue;
...
export function validateRustArtifact(artifact, expectedSource, platform = process.platform, arch = process.arch) {
    if (resolve(artifact.sourcePath) !== resolve(expectedSource)) {
        throw new Error(`compiler returned an artifact for ${artifact.sourcePath}, expected ${resolve(expectedSource)}`);
    }
    ...
    if (artifact.platform !== platform || artifact.arch !== arch) {
        throw new Error(`native artifact targets ${artifact.platform}-${artifact.arch}, but the runtime is ${platform}-${arch}`);
    }
}
async function compileOnce(sourcePath, options) {
    ...
    try {
        ...
        if (pendingCompilations.get(key) === pending)
            pendingCompilations.delete(key);
    }
}
function compilationKey(sourcePath, options) {
