...
import { prepareRustImports } from './build';
const options = parseArguments(process.argv.slice(2));
if (options.rewrite === 'in-place' && process.env.VERCEL !== '1' && !options.allowSourceMutation) {
    throw new Error('rust-imports: in-place rewriting is intended for a disposable build checkout. ' +
...
for (const artifact of result.artifacts) {
    console.log(`rust-imports: ${artifact.sourcePath} -> ${artifact.nodePath} (${artifact.platform}-${artifact.arch})`);
}
console.log(`rust-imports: prepared ${result.artifacts.length} native module(s) across ` +
...
function requiredValue(flag, inlineValue, _args, next) {
    const value = inlineValue ?? next();
    if (!value || value.startsWith('--'))
        throw new Error(`rust-imports: ${flag} requires a value`);
    return value;
}
function printHelp() {
